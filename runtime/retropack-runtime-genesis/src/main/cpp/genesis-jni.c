...
static struct {
    ...
    uint32_t video_buffer[GENESIS_WIDTH * GENESIS_HEIGHT];
    int video_width;
    int video_height;
    RingBuffer* audio_rb;
    ...
    .rom_loaded = false,
    .storage_path = {0},
    .video_width = GENESIS_WIDTH,
    .video_height = GENESIS_HEIGHT,
    .audio_rb = NULL,
...
Java_com_retropack_runtime_genesis_GenesisNativeCore_genesisInit(
    ...
    LOGI("Genesis Plus GX native runtime initialized");
...
Java_com_retropack_runtime_genesis_GenesisNativeCore_genesisLoadRom(
    ...
    g_genesis.video_width = GENESIS_WIDTH;
    g_genesis.video_height = GENESIS_HEIGHT;
    g_genesis.sram_size = 0x8000;
    g_genesis.sram_size_valid = true;

    LOGI("Genesis Plus GX loaded ROM: %s", native_path);
    pthread_mutex_unlock(&g_genesis.lock);
