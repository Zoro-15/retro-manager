...
static struct {
    ...
    uint32_t video_buffer[SNES_WIDTH * SNES_HIRES_HEIGHT];
    int video_width;
    int video_height;
    RingBuffer* audio_rb;
...
static void init_jni_cache(JNIEnv* env) {
    if (g_snes.byte_buffer_class) return;
    jclass bb_local = (*env)->FindClass(env, "java/nio/ByteBuffer");
    if (!bb_local) return;
    g_snes.byte_buffer_class = (jclass)(*env)->NewGlobalRef(env, bb_local);
    (*env)->DeleteLocalRef(env, bb_local);

    g_snes.byte_buffer_order = (*env)->GetMethodID(env, g_snes.byte_buffer_class,
        "order", "(Ljava/nio/ByteOrder;)Ljava/nio/ByteBuffer;");
    g_snes.byte_buffer_as_int_buffer = (*env)->GetMethodID(env, g_snes.byte_buffer_class,
    ...
    if (bo_class) {
        ...
        if (bo_native) {
            ...
            if (order_local) {
                g_snes.byte_order_native = (*env)->NewGlobalRef(env, order_local);
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesInit(
    ...
    (void) thiz;
    pthread_mutex_lock(&g_snes.lock);

    if (g_snes.initialized) {
        pthread_mutex_unlock(&g_snes.lock);
        return JNI_TRUE;
    }
    ...
    if (internalStoragePath) {
        ...
        if (path) {
            strncpy(g_snes.storage_path, path, sizeof(g_snes.storage_path) - 1);
            g_snes.storage_path[sizeof(g_snes.storage_path) - 1] = '\0';
            (*env)->ReleaseStringUTFChars(env, internalStoragePath, path);
        }
    }

    if (!g_snes.audio_rb) {
        g_snes.audio_rb = ringbuffer_create(AUDIO_BUFFER_CAPACITY);
    }

    init_jni_cache(env);
    g_snes.initialized = true;
    LOGI("Snes9x native runtime initialized");
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesLoadRom(
    ...
    pthread_mutex_lock(&g_snes.lock);
    g_snes.rom_loaded = true;
    g_snes.video_width = SNES_WIDTH;
    g_snes.video_height = SNES_HEIGHT;
    g_snes.sram_size = 0x8000;
    g_snes.sram_size_valid = true;

    LOGI("Snes9x loaded ROM: %s", native_path);
    pthread_mutex_unlock(&g_snes.lock);
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesUnloadRom(JNIEnv* env, jobject thiz) {
    (void) env; (void) thiz;
    pthread_mutex_lock(&g_snes.lock);
    g_snes.rom_loaded = false;
    if (g_snes.audio_rb) ringbuffer_reset(g_snes.audio_rb);
    pthread_mutex_unlock(&g_snes.lock);
}
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesDestroy(JNIEnv* env, jobject thiz) {
    ...
    pthread_mutex_lock(&g_snes.lock);
    if (g_snes.audio_rb) {
        ringbuffer_destroy(g_snes.audio_rb);
        g_snes.audio_rb = NULL;
    }
    g_snes.initialized = false;
    g_snes.rom_loaded = false;
    pthread_mutex_unlock(&g_snes.lock);
}
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesSetKeys(
    ...
    (void) env; (void) thiz;
    g_snes.key_mask = (uint32_t) keyMask;
}
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesGetVideoBuffer(JNIEnv* env, jobject thiz) {
    ...
    jlong byte_capacity = (jlong)(g_snes.video_width * g_snes.video_height * sizeof(uint32_t));
    jobject direct_bb = (*env)->NewDirectByteBuffer(env, g_snes.video_buffer, byte_capacity);
    if (!direct_bb || !g_snes.byte_buffer_class || !g_snes.byte_buffer_as_int_buffer) {
        pthread_mutex_unlock(&g_snes.lock);
        return NULL;
    }

    if (g_snes.byte_order_native && g_snes.byte_buffer_order) {
        (*env)->CallObjectMethod(env, direct_bb, g_snes.byte_buffer_order, g_snes.byte_order_native);
    }
    jobject int_buffer = (*env)->CallObjectMethod(env, direct_bb, g_snes.byte_buffer_as_int_buffer);
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesGetAudioSamples(
    ...
    (void) thiz;
    if (!outSamples || maxSamples <= 0 || !g_snes.audio_rb) return 0;
    jshort* dst = (*env)->GetPrimitiveArrayCritical(env, outSamples, NULL);
    if (!dst) return 0;
    size_t read = ringbuffer_read(g_snes.audio_rb, (int16_t*) dst, (size_t) maxSamples);
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesGetAudioAvailable(JNIEnv* env, jobject thiz) {
    (void) env; (void) thiz;
    if (!g_snes.audio_rb) return 0;
    return (jint) ringbuffer_available(g_snes.audio_rb);
}
...
Java_com_retropack_runtime_snes_Snes9xNativeCore_snesGetVideoSize(JNIEnv* env, jobject thiz) {
    ...
    if (result) {
        jint dims[2] = { g_snes.video_width, g_snes.video_height };
